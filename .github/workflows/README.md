...
Examples: `v1.0.1`, `v1.0.2-beta1` etc.
...
### For all tags:
- `densify/azure-rest-api-token:${tag}`

### For official releases only (format: #.#.#):
- `densify/azure-rest-api-token:latest`

### Official vs Non-Official Releases

**Official releases** follow the pattern `#.#.#` (e.g., `1.0.1`, `1.1.0`) and will also get the `latest` tag.

**Non-official releases** like `1.0.2-beta1`, `1.0.2-alpha`, `1.0.2-rc1` will only get the exact tag, not the `latest` tag. This ensures that customers pulling `latest` always get the latest stable release, not a pre-release version.

## Setup

### Prerequisites

1. **Docker Hub Account**: You need a Docker Hub account with push permissions to the `densify/azure-rest-api-token` repository.
...
git tag v1.0.2-beta1
git push origin v1.0.2-beta1
```

### Examples

**Official Release `1.0.2`** creates these tags:
- `densify/azure-rest-api-token:1.0.2`
- `densify/azure-rest-api-token:latest` ← Updates `latest`

**Beta Release `1.0.2-beta1`** creates only:
- `densify/azure-rest-api-token:1.0.2-beta1`
...
3. Download the `sbom-${tag}` artifact (e.g., `sbom-1.0.2`)
...
**Official Releases** (will create `latest` tag):
- `4.2.2`, `1.0.0`, `10.5.3` - Standard semantic versioning

**Pre-releases** (exact tag only, no major version updates):
- `1.0.2-beta1`, `1.0.2-alpha`, `1.0.2-rc1` - Beta, alpha, release candidates
- `1.0.2-hotfix`, `1.0.2.1` - Hotfixes or patch releases with additional identifiers

This ensures that `latest` tag always points to the latest stable release, never to pre-release versions.
